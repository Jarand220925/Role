 ...
 * @author Jarand
 */
public class Sea extends GameObject implements LandscapeObject {
    ...
    public Sea(float x, float y, int type, ObjectId id) {
        super(x, y, id);
        this.type = type;
    }
    ...
    vision();    
        ...
        if(type == 1 && seen == true) //sea
        {
            g.drawImage(tex.block[1], (int)x, (int)y, 96, 96, null);
        } else {
    ...
    float rangey = 500.0f;
        ...
        if(px >= newx - rangex && px <= newx + rangex && py >= newy - rangey && py <= newy + rangey){
        this.seen = true;
        } else {
        this.seen = false;
        }
